...
import java.io.BufferedInputStream; //IO
import java.io.FileOutputStream;
import java.io.InputStream;
import java.net.URL; //네트워크
import java.net.URLConnection;
import java.util.Date; //객체지향
...
class getimage{
	...
	public void img() throws Exception{
		...
		this.is = u.openStream(); //통신에선 openStream 만 사용
		this.bi = new BufferedInputStream(this.is); //이미지 파일을 임시로 메모리에 저장
		//byte data[] = new byte[this.bi.available()]; //속도가 늦어질 수 있음.
		...
		int imgdata = 0;
		int count = 0;
		while((imgdata=this.bi.read(data))!=-1) { //readLine 이면 null 써줘야한다, 지금은 숫자니까 -1넣어줌
