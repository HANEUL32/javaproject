...
class ch_server extends Thread{
	...
	public ch_server(Socket s) { //setter 기본 옵션을 모두 셋팅
		...
		try { //모든 필드값에 속성값을 전달
			...
			this.is = this.sk.getInputStream(); //읽기
			this.isr = new InputStreamReader(this.is); //byte -> Strings
	...
	public void run() { //스레드 작동
		...
		try {
			...
			info("["+name+"] Welcome"); //클라이언트에 환영 메세지 출력 1회
			
			while(this.br != null) {
				//클라이언트 입력한 메세지를 전달(배열에 있는 모든 사용자)
